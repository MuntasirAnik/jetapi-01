...
export class FeatureFlagGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const enabled = flags[flag] !== undefined ? flags[flag] : (this.DEFAULTS[flag] ?? true);

    if (!enabled) {
      throw new ForbiddenException(`This feature is currently disabled by the administrator.`);
    }
    return true;
  }
}
