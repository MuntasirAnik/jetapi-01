...
export class CollectionPluginsController {
  ...
  async getCollectionPlugins(
    ...
    const availablePlugins = await this.pluginRepo.find({
      where: { enabled: true },
      order: { category: 'ASC', name: 'ASC' },
    });

    // Get user's plugin activations for this collection
    const userPlugins = await this.userPluginRepo.find({
  ...
  async updateCollectionPlugin(
    ...
    let userPlugin = await this.userPluginRepo.findOne({
      where: { userId, collectionId, pluginSlug: slug },
    });
  ...
  async removeCollectionPlugin(
    ...
    const userId = req.user.sub;
    const result = await this.userPluginRepo.delete({
      userId,
      collectionId,
      pluginSlug: slug,
    });
    if (result.affected === 0)
      throw new NotFoundException('Plugin not activated for this collection');
    return { removed: true };
  }
  ...
  async testCollectionPlugin(
    ...
    let config: any = {};
    try {
      config = JSON.parse(userPlugin.config);
    } catch {}
    ...
    try {
      switch (slug) {
        ...
        case 'zapier':
          ...
          return {
            success: true,
            message: 'Webhook URL configured. Ready to send events.',
          };
        case 'smtp':
