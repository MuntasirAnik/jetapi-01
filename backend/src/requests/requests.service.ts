...
export class RequestsService {
  ...
  private async getShareRole(
    collectionId: string,
    userId: string,
  ): Promise<'owner' | 'admin' | 'editor' | 'viewer' | null> {
    const collection = await this.requestRepository.manager
    ...
    const share = (collection as any).shares?.find(
    ...
    if (collection.workspace) {
      const membership = await this.requestRepository.manager
        .getRepository('OrganizationUser')
        .findOne({
          where: {
            organizationId: collection.workspace.organizationId,
            userId,
          },
        });
      if (
  ...
  async create(
    ...
    if (data.collectionId) {
      ...
      if (role && !this.canEdit(role)) {
        throw new ForbiddenException(
          'You have view-only access to this collection. You cannot add endpoints.',
        );
      }
    }
    ...
    if (data.name && data.collectionId) {
      ...
      if (existing) {
        throw new BadRequestException(
          'An endpoint with this name already exists in this folder.',
        );
      }
    }
    ...
    // Log activity
    try {
      const user = await this.requestRepository.manager
        .getRepository('User')
  ...
  async findAll(userId: string): Promise<RequestItem[]> {
    return this.requestRepository.find({
      where: [{ ownerId: userId }, { ownerId: IsNull() as any }],
    });
  }
  ...
  async findOne(id: string, userId: string): Promise<RequestItem> {
    ...
    if (request.collection) {
      ...
      if (
        (request.collection as any).shares?.some(
          (s: any) => s.userId === userId,
        )
      )
        hasAccess = true;
      ...
      if (!hasAccess && request.collection.workspace) {
        const membership = await this.requestRepository.manager
          ...
          .findOne({
            where: {
              organizationId: request.collection.workspace.organizationId,
              userId,
            },
          });
  ...
  async update(
    ...
    if (request.collectionId) {
      ...
      if (role && !this.canEdit(role)) {
        throw new ForbiddenException(
          'You have view-only access to this collection. You cannot edit endpoints.',
        );
      }
    }
    ...
    if (data.name && data.name !== request.name) {
      const existing = await this.requestRepository.findOne({
        where: {
          ...
          folder: data.folder ?? request.folder ?? '',
        },
      });
      if (existing) {
        throw new BadRequestException(
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (request.collectionId) {
      ...
      if (role && !this.canEdit(role)) {
        throw new ForbiddenException(
          'You have view-only access to this collection. You cannot delete endpoints.',
        );
      }
    }
  ...
  async restore(id: string, userId: string): Promise<RequestItem> {
    ...
    if (!request.deletedAt)
      throw new BadRequestException('Request is not in trash');

    await this.requestRepository.recover(request);
    return this.requestRepository.findOne({
      where: { id },
    }) as Promise<RequestItem>;
  }
  ...
  async moveRequest(
    ...
    if (request.collectionId) {
      ...
      if (role && !this.canEdit(role)) {
        throw new ForbiddenException(
          'You have view-only access to the source collection.',
        );
      }
    }
    ...
    if (newCollectionId && newCollectionId !== request.collectionId) {
      ...
      if (targetRole && !this.canEdit(targetRole)) {
        throw new ForbiddenException(
          'You have view-only access to the target collection.',
        );
      }
      ...
      if (existing) {
        throw new BadRequestException(
          `A request named "${request.name}" already exists in the target collection.`,
        );
      }
  ...
  async moveFolder(
    ...
    if (
      !isCrossCollection &&
      (newPath === oldPath || newPath.startsWith(oldPath + '/'))
    ) {
