...
export class EnvironmentsService {
  ...
  private async canAccessWorkspace(
    ...
    const workspace = await this.environmentRepository.manager
      .getRepository('Workspace')
      .findOne({ where: { id: workspaceId } });
    if (!workspace) return false;

    // Check organization membership
    const membership = await this.environmentRepository.manager
    ...
    const workspaceCollections = await this.environmentRepository.manager
  ...
  async findOne(id: string, userId: string): Promise<Environment> {
    ...
    const hasAccess = await this.canAccessWorkspace(
      environment.workspaceId,
      userId,
    );
    if (!hasAccess)
      throw new NotFoundException('Environment not found or access denied');
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    if (environment.ownerId && environment.ownerId !== userId) {
      const workspace = await this.environmentRepository.manager
        .getRepository('Workspace')
        .findOne({ where: { id: environment.workspaceId } });
      if (workspace) {
        const membership = await this.environmentRepository.manager
          .getRepository('OrganizationUser')
          .findOne({
        ...
        if (
          ...
          throw new ForbiddenException(
            'Only workspace admins or the creator can delete this environment',
          );
        }
      }
    }
