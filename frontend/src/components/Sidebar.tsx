...
export default function Sidebar({ workspaces = [], activeWorkspace, sharedCollections = [], onSelectRequest, activeRequestId }: any) {
  ...
  useEffect(() => {
    if (history.length > 0) localStorage.setItem('jetapi_history', JSON.stringify(history));
    else localStorage.removeItem('jetapi_history');
  }, [history]);
  ...
  useEffect(() => {
    const handler = (e: any) => {
      ...
      if (!entry) return;
      setHistory(prev => [{ ...entry, id: `hist-${Date.now()}-${Math.random().toString(36).slice(2,6)}` }, ...prev].slice(0, 100));
    };
    window.addEventListener('jetapi-history-push', handler);
    return () => window.removeEventListener('jetapi-history-push', handler);
  }, []);
    ...
    <div className="flex h-full w-full bg-[var(--sidebar)] border-r border-[var(--border)] group/rail">
      ...
      <div className="w-[54px] flex-shrink-0 border-r border-[var(--border)] bg-[var(--background)] flex flex-col items-center py-4 gap-2 text-[var(--muted)] z-10 selection:bg-transparent justify-start">
         ...
         >
            ...
            {history.length > 0 && (
              <span className="absolute top-1.5 right-2 w-2 h-2 rounded-full bg-[var(--color-brand-500)]"></span>
            )}
         ...
         >
            ...
            <span className="absolute left-[60px] bg-[var(--foreground)] text-[var(--background)] px-2.5 py-1 rounded text-[10px] font-bold opacity-0 group-hover:opacity-100 pointer-events-none z-50 transition-opacity whitespace-nowrap shadow-xl">Activity</span>
         </button>
      </div>
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden">
            <div className="p-3 pb-2 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-xs tracking-wide">History</span>
                {history.length > 0 && (
                  <span className="text-[10px] text-[var(--muted)] font-mono">{history.length}</span>
                )}
              </div>
              {history.length > 0 && (
                <button 
                  onClick={() => { setHistory([]); toast.success('History cleared'); }}
                  className="p-1 hover:bg-[var(--card)] hover:text-red-500 text-[var(--muted)] rounded cursor-pointer transition-colors"
                  title="Clear History"
                >
            ...
            <div className="flex-1 overflow-y-auto px-1">
              {history.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-[var(--muted)] gap-2">
                  ...
                  <p className="text-[10px] opacity-60 max-w-[180px] text-center">Send a request and it will appear here automatically.</p>
                </div>
              ) : (
                (() => {
                  ...
                  return Object.entries(groups).map(([label, items]) => (
                    <div key={label} className="mb-3">
                      <div className="text-[10px] font-bold uppercase tracking-wider text-[var(--muted)] px-2 py-1 sticky top-0 bg-[var(--sidebar)] z-10">{label}</div>
                      {items.map((entry: any) => {
                        const statusColor = !entry.status ? 'text-[var(--muted)]' : entry.status < 300 ? 'text-green-500' : entry.status < 400 ? 'text-yellow-500' : 'text-red-500';
                        const methodColor = entry.method === 'GET' ? 'text-green-500' : entry.method === 'POST' ? 'text-orange-500' : entry.method === 'PUT' ? 'text-blue-500' : entry.method === 'DELETE' ? 'text-red-500' : entry.method === 'PATCH' ? 'text-yellow-500' : 'text-[var(--foreground)]';
                        ...
                        return (
                          <div 
                            ...
                            onClick={() => {
                              if (entry.request) {
                                const reopened = { ...entry.request, id: `hist-${Date.now()}`, _isNew: true, name: entry.name || 'History Request' };
                                onSelectRequest(reopened);
                              }
                            }}
                            className="flex items-center gap-2 px-2 py-1.5 hover:bg-[var(--card)] rounded cursor-pointer group transition-colors"
                          >
                            <span className={`font-mono text-[10px] font-bold shrink-0 w-9 ${methodColor}`}>{entry.method?.substring(0,4)}</span>
                            <div className="flex-1 min-w-0">
                              <div className="text-xs truncate text-[var(--foreground)]">{entry.name || entry.url}</div>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className={`text-[10px] font-bold ${statusColor}`}>{entry.status || '—'}</span>
                                <span className="text-[10px] text-[var(--muted)]">{entry.timeMs ? `${entry.timeMs}ms` : ''}</span>
                                <span className="text-[10px] text-[var(--muted)] opacity-60 ml-auto">{time}</span>
                              </div>
                            </div>
                          </div>
                        );
